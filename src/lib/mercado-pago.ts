...
export type CheckoutAvailabilityResponse = {
...
export async function getCheckoutAvailability(
  session: Session,
  input: CheckoutAvailabilityInput,
) {
  if (!SUPABASE_PUBLISHABLE_KEY) {
    throw new Error("A chave publica do Supabase nao esta configurada.");
  }
  ...
  const activeSession = await getVerifiedSession(session);
  const response = await fetch(`${SUPABASE_FUNCTIONS_URL}/checkout-availability`, {
    body: JSON.stringify(input),
    headers: {
      apikey: SUPABASE_PUBLISHABLE_KEY,
      Authorization: `Bearer ${activeSession.access_token}`,
  ...
  const data = (await response.json().catch(() => null)) as CheckoutAvailabilityResponse | null;

  if (!data) {
    throw new Error("Nao foi possivel verificar a disponibilidade da promocao.");
  }
