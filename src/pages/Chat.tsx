...
const CHAT_TERMS_ITEMS = [
  {
    body: "Converse com respeito. Nao e permitido discurso de odio, assedio, ameacas, golpes ou spam.",
    title: "Respeito entre participantes",
  },
  {
    body: "Nao compartilhe dados pessoais, links suspeitos, PIX, telefone, senha ou propostas de pagamento fora da plataforma.",
    title: "Seguranca da comunidade",
  },
  {
    body: "As mensagens podem ser moderadas e registradas para fins de auditoria, seguranca e suporte operacional.",
    title: "Moderacao e auditoria",
  },
  {
    body: "Condutas ilicitas ou abusivas podem gerar bloqueio do chat, banimento da conta e cancelamento de participacoes em sorteios vigentes.",
    title: "Consequencias",
  },
];
...
function getPublicMessageDisplayBody(message: Pick<ChatEntry, "body" | "isHidden">) {
  return message.isHidden ? "Oculta pela moderacao" : message.body;
}

function getRestrictedPublicChatError(message: string) {
  ...
  const containsLink =
    /(^|[\s\p{P}])((https?:\/\/)|(www\.))\S+/iu.test(trimmedMessage) ||
    /(^|[\s\p{P}])[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.(com|com\.br|net|org|io|me|co|app|site|xyz|info|biz|dev|gg|ly|tv|online|store|blog|link|shop|br)\b/iu.test(
      trimmedMessage,
    );

  if (containsLink) {
    return "Nao e permitido enviar links no chat publico.";
  }

  if (/(^|[^0-9])\d{3}\.?\d{3}\.?\d{3}-?\d{2}([^0-9]|$)/u.test(trimmedMessage)) {
    return "Nao e permitido enviar CPF no chat publico.";
  }

  if (/(^|[^0-9])(?:\+?55\s*)?(?:\(?\d{2}\)?\s*)9?\d{4}[-\s]?\d{4}([^0-9]|$)/u.test(trimmedMessage)) {
    return "Nao e permitido enviar telefone no chat publico.";
  }
...
const TOP_LOAD_BUTTON_THRESHOLD = 96;
const BOTTOM_STICK_THRESHOLD = 72;

function isNearTop(element: HTMLDivElement | null) {
  return Boolean(element) && element.scrollTop <= TOP_LOAD_BUTTON_THRESHOLD;
}
...
export default function Chat() {
  ...
  const [adminPublicSenderIds, setAdminPublicSenderIds] = useState<Set<string>>(new Set());
  ...
  const [loadingOlderPublic, setLoadingOlderPublic] = useState(false);
  const [loadingOlderPrivate, setLoadingOlderPrivate] = useState(false);
  const [publicHasOlderMessages, setPublicHasOlderMessages] = useState(false);
  const [privateHasOlderMessages, setPrivateHasOlderMessages] = useState(false);
  const [showPublicLoadOlder, setShowPublicLoadOlder] = useState(false);
  const [showPrivateLoadOlder, setShowPrivateLoadOlder] = useState(false);
  ...
  const [chatTermsAcceptedAt, setChatTermsAcceptedAt] = useState<string | null>(null);
  const [chatTermsLoading, setChatTermsLoading] = useState(true);
  const [chatTermsDialogOpen, setChatTermsDialogOpen] = useState(false);
  const [acceptingChatTerms, setAcceptingChatTerms] = useState(false);
  const scrollRef = useRef<HTMLDivElement>(null);
  const privateScrollRef = useRef<HTMLDivElement>(null);
  const publicRestoreScrollHeightRef = useRef<number | null>(null);
  const privateRestoreScrollHeightRef = useRef<number | null>(null);
  const shouldScrollPublicToBottomRef = useRef(true);
  const shouldScrollPrivateToBottomRef = useRef(true);
  const resolvedPublicSenderIdsRef = useRef<Set<string>>(new Set());
  ...
  const hasAcceptedChatTerms = Boolean(chatTermsAcceptedAt);
  const isChatModerator = participantControls.moderator;
  ...
  async function syncPublicAdminSenders(userIds: string[]) {
    const uniqueUserIds = Array.from(new Set(userIds.filter(Boolean)));
    const pendingUserIds = uniqueUserIds.filter((userId) => !resolvedPublicSenderIdsRef.current.has(userId));

    if (!pendingUserIds.length) {
      return;
    }

    const adminUserIds = await loadAdminRoleUserIds(supabase, pendingUserIds);

    pendingUserIds.forEach((userId) => resolvedPublicSenderIdsRef.current.add(userId));

    if (adminUserIds.size === 0) {
      return;
    }

    setAdminPublicSenderIds((current) => {
      const next = new Set(current);
      adminUserIds.forEach((userId) => next.add(userId));
      return next;
    });
  }

  async function markPrivateMessagesAsRead(messages: ChatEntry[]) {
    if (!user) {
      return;
    }
  ...
  async function fetchPublicChatPage(beforeSentAt?: string | null) {
    let query = supabase
      .from("public_chat_messages")
      .select("*")
      .order("sent_at", { ascending: false })
      .limit(PUBLIC_CHAT_PAGE_SIZE + 1);
    ...
    const hasOlderMessages = normalized.length > PUBLIC_CHAT_PAGE_SIZE;
    const messages = hasOlderMessages ? normalized.slice(-PUBLIC_CHAT_PAGE_SIZE) : normalized;
  ...
  async function fetchPrivateChatPage(beforeSentAt?: string | null, supportUserId?: string | null) {
    ...
    let query = supabase
      ...
      .select("*")
      .or(
        `and(sender_id.eq.${user.id},receiver_id.eq.${supportUserId}),and(sender_id.eq.${supportUserId},receiver_id.eq.${user.id})`,
      )
      .order("sent_at", { ascending: false })
      .limit(PRIVATE_CHAT_PAGE_SIZE + 1);
    ...
    const hasOlderMessages = normalized.length > PRIVATE_CHAT_PAGE_SIZE;
    const messages = hasOlderMessages ? normalized.slice(-PRIVATE_CHAT_PAGE_SIZE) : normalized;
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .then(({ data, error }) => {
        ...
        const acceptedAt =
          typeof data?.chat_terms_accepted_at === "string" && data.chat_terms_accepted_at.trim()
            ? data.chat_terms_accepted_at
            : null;

        setChatTermsAcceptedAt(acceptedAt);
        setChatTermsDialogOpen(!acceptedAt);
        setChatTermsLoading(false);
      });
  ...
  useEffect(() => {
    resolvedPublicSenderIdsRef.current = new Set();
    setAdminPublicSenderIds(new Set());
  }, [user?.id]);
  ...
  useEffect(() => {
    ...
    async function loadPublicChat() {
      ...
      const { error, hasOlderMessages, messages } = await fetchPublicChatPage();
      ...
      shouldScrollPublicToBottomRef.current = true;
      setPublicMessages(messages);
      setPublicHasOlderMessages(hasOlderMessages);
      setPublicLoading(false);
      void syncIdentities(messages.map((message) => message.senderId));
      void syncPublicAdminSenders(messages.map((message) => message.senderId));
    }

    async function loadPrivateChat() {
      setPrivateLoading(true);
      const { error, hasOlderMessages, messages } = await fetchPrivateChatPage(null, supportReceiverId);
      ...
      shouldScrollPrivateToBottomRef.current = true;
      setPrivateMessages(messages);
      setPrivateHasOlderMessages(hasOlderMessages);
      setPrivateLoading(false);
      void syncIdentities(
        messages.flatMap((message) => [message.senderId, message.receiverId ?? ""]).filter(Boolean),
      );
      await markPrivateMessagesAsRead(messages);
    }
    ...
    if (supportReceiverId) {
      ...
      void loadPublicChat();
      setPrivateMessages([]);
      setPrivateHasOlderMessages(false);
      setPrivateLoading(false);
    }
    ...
    const publicChannel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "public_chat_messages" }, (payload) => {
        ...
        shouldScrollPublicToBottomRef.current = isNearBottom(scrollRef.current);
        setPublicMessages((current) => mergeChatEntries(current, [message]));
        void syncIdentities([message.senderId]);
        void syncPublicAdminSenders([message.senderId]);
      })
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "public_chat_messages" }, (payload) => {
        const message = normalizePublicChatMessage(payload.new);
    ...
    const privateChannel = supabase
      .channel(`private-chat:${user.id}:${supportReceiverId ?? "pending"}`)
      .on(
        "postgres_changes",
        ...
        (payload) => {
          ...
          if (
            ...
            !isActive ||
            !supportReceiverId ||
            !isSupportConversationMessage(message, user.id, supportReceiverId)
          ) {
            return;
          }

          shouldScrollPrivateToBottomRef.current = isNearBottom(privateScrollRef.current);
          setPrivateMessages((current) => mergeChatEntries(current, [message]));
  ...
  const filteredPublicMessages = useMemo(() => {
    ...
    return publicMessages.filter((message) => {
      const senderName =
        adminPublicSenderIds.has(message.senderId)
      ...
      const displayBody = getPublicMessageDisplayBody(message).toLowerCase();

      return senderName.includes(query) || displayBody.includes(query);
    });
  }, [adminPublicSenderIds, currentUserIdentity.name, participantIdentities, publicMessages, publicSearch, user?.id]);
  ...
  useEffect(() => {
    ...
    if (publicRestoreScrollHeightRef.current !== null) {
      const previousScrollHeight = publicRestoreScrollHeightRef.current;
      publicRestoreScrollHeightRef.current = null;
      container.scrollTop += container.scrollHeight - previousScrollHeight;
      setShowPublicLoadOlder(publicHasOlderMessages && isNearTop(container));
      return;
    }

    if (shouldScrollPublicToBottomRef.current) {
      shouldScrollPublicToBottomRef.current = false;
      container.scrollTo({ behavior: "smooth", top: container.scrollHeight });
    }

    setShowPublicLoadOlder(publicHasOlderMessages && isNearTop(container));
  }, [publicHasOlderMessages, publicMessages]);

  useEffect(() => {
    const container = privateScrollRef.current;

    if (!container) {
      return;
    }

    if (privateRestoreScrollHeightRef.current !== null) {
      const previousScrollHeight = privateRestoreScrollHeightRef.current;
      privateRestoreScrollHeightRef.current = null;
      container.scrollTop += container.scrollHeight - previousScrollHeight;
      setShowPrivateLoadOlder(privateHasOlderMessages && isNearTop(container));
      return;
    }

    if (shouldScrollPrivateToBottomRef.current) {
      shouldScrollPrivateToBottomRef.current = false;
      container.scrollTo({ behavior: "smooth", top: container.scrollHeight });
    }

    setShowPrivateLoadOlder(privateHasOlderMessages && isNearTop(container));
  }, [privateHasOlderMessages, privateMessages]);
  ...
  }, [privateHasOlderMessages]);

  async function loadOlderPublicMessages() {
    if (loadingOlderPublic || publicLoading || !publicHasOlderMessages) {
      return;
    }

    const beforeSentAt = publicMessages[0]?.createdAt;

    if (!beforeSentAt) {
      setPublicHasOlderMessages(false);
      return;
    }

    publicRestoreScrollHeightRef.current = scrollRef.current?.scrollHeight ?? null;
    setLoadingOlderPublic(true);

    const { error, hasOlderMessages, messages } = await fetchPublicChatPage(beforeSentAt);

    if (error) {
      publicRestoreScrollHeightRef.current = null;
      setLoadingOlderPublic(false);
      toast.error("Nao foi possivel carregar mensagens antigas do chat publico.");
      return;
    }

    setPublicMessages((current) => mergeChatEntries(messages, current));
    setPublicHasOlderMessages(hasOlderMessages);
    setLoadingOlderPublic(false);
    void syncIdentities(messages.map((message) => message.senderId));
  }
  ...
  async function loadOlderPrivateMessages() {
    if (loadingOlderPrivate || privateLoading || !privateHasOlderMessages) {
      return;
    }

    const beforeSentAt = privateMessages[0]?.createdAt;
    ...
    privateRestoreScrollHeightRef.current = privateScrollRef.current?.scrollHeight ?? null;
    setLoadingOlderPrivate(true);

    const { error, hasOlderMessages, messages } = await fetchPrivateChatPage(beforeSentAt, supportReceiverId);

    if (error) {
      privateRestoreScrollHeightRef.current = null;
      setLoadingOlderPrivate(false);
      toast.error("Nao foi possivel carregar mensagens antigas do suporte.");
      return;
    }

    setPrivateMessages((current) => mergeChatEntries(messages, current));
    setPrivateHasOlderMessages(hasOlderMessages);
    setLoadingOlderPrivate(false);
    void syncIdentities(
      messages.flatMap((message) => [message.senderId, message.receiverId ?? ""]).filter(Boolean),
  ...
  async function handleAcceptChatTerms() {
    if (!user || acceptingChatTerms) return;

    const acceptedAt = new Date().toISOString();
    setAcceptingChatTerms(true);
    ...
    setAcceptingChatTerms(false);

    if (error) {
      toast.error("Nao foi possivel registrar o aceite das regras agora.");
      return;
    }

    setChatTermsAcceptedAt(acceptedAt);
    setChatTermsDialogOpen(false);
    toast.success("Regras do chat aceitas.");
  }

  async function handleSendPublicMessage() {
    if (!user || !newMessage.trim() || sendingPublic) return;
    const restrictedContentError = getRestrictedPublicChatError(newMessage);

    if (!hasAcceptedChatTerms) {
      setChatTermsDialogOpen(true);
      toast.error("Aceite as regras do chat antes de enviar mensagens.");
      return;
    }
    ...
    if (restrictedContentError) {
      toast.error(restrictedContentError);
      return;
    }
  ...
  async function handleHideReportedMessage() {
    if (!user || !reportingMessage || !isChatModerator || submittingReport) return;

    setSubmittingReport(true);
    const { data, error } = await supabase.rpc("hide_public_chat_message", { _message_id: reportingMessage.id });
    setSubmittingReport(false);

    if (error) {
      toast.error("Nao foi possivel ocultar a mensagem agora.");
      return;
    }

    const hiddenRow = Array.isArray(data) ? data[0] : data;
    const hiddenAt = typeof hiddenRow?.hidden_at === "string" ? hiddenRow.hidden_at : new Date().toISOString();
    const hiddenBy = typeof hiddenRow?.hidden_by === "string" ? hiddenRow.hidden_by : user.id;
    ...
    setReportReason("");
    setReportingMessage(null);
    toast.success("Mensagem ocultada pela moderacao.");
  }
      ...
      <main className="container mx-auto max-w-6xl px-4 pb-10 pt-24">
        <div className="mb-6 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-display font-bold">Chat</h1>
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
              Troque ideias com outros participantes e acompanhe tudo sobre os sorteios em tempo real.
            </p>
          </div>
        ...
        <Tabs className="space-y-4" defaultValue="public">
              ...
              >
                ...
                <div className="overflow-hidden rounded-[28px] border border-border/60 bg-background/75 shadow-sm">
                  <div
                    ...
                    ref={scrollRef}
                  >
                    {publicHasOlderMessages && (showPublicLoadOlder || loadingOlderPublic) ? (
                      <div className="sticky top-0 z-10 flex justify-center pb-2">
                        <Button
                          className="rounded-full border-border/60 bg-background/95 shadow-sm backdrop-blur"
                          disabled={loadingOlderPublic}
                          onClick={() => void loadOlderPublicMessages()}
                          size="sm"
                          type="button"
                          variant="outline"
                        >
                          {loadingOlderPublic ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                          {loadingOlderPublic ? "Carregando..." : "Carregar mensagens antigas"}
                        </Button>
                      </div>
                    ) : null}
                    ...
                    {publicLoading ? (
                      ...
                      filteredPublicMessages.map((message) => {
                        const isAdminNotice = adminPublicSenderIds.has(message.senderId);
                        const isOwnMessage = !isAdminNotice && message.senderId === user.id;
                        const identity = isAdminNotice
                          ? {
                              avatarUrl: null,
                              bubbleThemeId: null,
                              name: "Equipe",
                        ...
                        const bubbleClass = message.isHidden
                          ? "border border-border/60 bg-zinc-950 text-zinc-100"
                          : getMessageBubbleClass(identity.bubbleThemeId, !isOwnMessage);
                        const displayBody = getPublicMessageDisplayBody(message);

                        if (isAdminNotice) {
                          return (
                            <div className="py-1" key={message.id}>
                              <div className="w-full rounded-md border border-primary/20 bg-primary/10 px-4 py-4 shadow-sm">
                        ...
                        <Popover>
                          <PopoverTrigger asChild>
                            <Button
                              ...
                              disabled={participantControls.publicChatBlocked || chatTermsLoading || !hasAcceptedChatTerms}
                              type="button"
                              variant="outline"
                            >
                        ...
                        <Input
                          ...
                          placeholder={
                            participantControls.publicChatBlocked
                              ...
                              : chatTermsLoading
                                ? "Validando regras do chat..."
                                : !hasAcceptedChatTerms
                                  ? "Aceite as regras do chat para participar"
          ...
          <TabsContent className="mt-0" value="private">
            <section className="rounded-[28px] border border-border/60 bg-card/70 p-4 shadow-sm sm:p-5">
              ...
              >
                {privateHasOlderMessages && (showPrivateLoadOlder || loadingOlderPrivate) ? (
                  <div className="sticky top-0 z-10 flex justify-center pb-2">
                    <Button
                      className="rounded-full border-border/60 bg-background/95 shadow-sm backdrop-blur"
                      disabled={loadingOlderPrivate}
                      onClick={() => void loadOlderPrivateMessages()}
                      size="sm"
                      type="button"
                      variant="outline"
                    >
                      {loadingOlderPrivate ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                      {loadingOlderPrivate ? "Carregando..." : "Carregar mensagens antigas"}
                    </Button>
                  </div>
                ) : null}
                ...
                <div className="flex flex-1 gap-2">
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        className="h-12 w-12 rounded-2xl border-border/60"
                        disabled={chatTermsLoading || !hasAcceptedChatTerms}
                        type="button"
                        variant="outline"
                      >
                        <Smile className="h-4 w-4" />
                  ...
                  <Input
                    ...
                    placeholder={
                      chatTermsLoading
                        ? "Validando regras do chat..."
                        : !hasAcceptedChatTerms
                          ? "Aceite as regras do chat para falar com a equipe"
        ...
        >
          ...
          >
            <DialogHeader>
              <DialogTitle>Regras do chat</DialogTitle>
            </DialogHeader>
            <div className="space-y-4 pt-2">
              <p className="text-sm leading-6 text-muted-foreground">
                Para participar da conversa, voce precisa aceitar as regras de uso e moderacao do chat.
              </p>

              <div className="rounded-[24px] border border-border/60 bg-card/70 p-4">
                {CHAT_TERMS_ITEMS.map((item) => (
                  <div
              ...
              <div className="rounded-2xl border border-amber-400/30 bg-amber-50/10 px-4 py-3 text-sm leading-6 text-amber-500">
                Para sua seguranca, este chat e moderado. Praticas ilicitas podem gerar banimento e reporte as autoridades.
              </div>

              <div className="flex flex-wrap justify-end gap-2">
                {hasAcceptedChatTerms ? (
                  <Button onClick={() => setChatTermsDialogOpen(false)} type="button" variant="outline">
                    Fechar
                  </Button>
                ) : null}
                <Button
                  disabled={acceptingChatTerms}
                  onClick={() => void handleAcceptChatTerms()}
                  type="button"
                  variant="hero"
                >
                  {acceptingChatTerms ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
            ...
            <div className="space-y-4 pt-2">
              <div className="rounded-2xl border border-amber-400/40 bg-amber-50/10 p-4">
                ...
                <p className="mt-2 text-sm text-foreground">
                  {reportingMessage ? getPublicMessageDisplayBody(reportingMessage) : ""}
                </p>
              </div>
              ...
              <div className="flex justify-end gap-2">
                {isChatModerator && !reportingMessage?.isHidden ? (
                  <Button
                    disabled={!reportingMessage || submittingReport}
                    onClick={() => void handleHideReportedMessage()}
                    type="button"
                    variant="outline"
                  >
                    Ocultar mensagem
