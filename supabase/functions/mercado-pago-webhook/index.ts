...
function buildPaymentStatusVariants(args: {
  amount: number;
  normalizedStatus: "failed" | "paid" | "pending";
  occurredAt: string;
  paymentId: number | string;
  ...
  if (args.normalizedStatus === "pending") {
    return [
      {
        amount: args.amount,
        fulfillment_error: null,
...
serve(async (request) => {
  ...
  const variants = buildPaymentStatusVariants({
    amount,
    normalizedStatus,
    occurredAt,
  ...
  if (normalizedStatus === "paid") {
    ...
    if (assignmentError) {
      console.error("Failed to assign promotion numbers for payment", paymentRecordId, assignmentError);

      const reviewError = await updatePaymentWithFallback(adminClient, paymentRecordId, [
        {
          fulfillment_error: assignmentError.message ?? "Could not assign the promotion numbers.",
          fulfillment_status: "manual_review",
          numbers_assigned_at: null,
          reservation_expires_at: null,
          status: "paid",
        },
        {
      ...
      if (reviewError) {
        return jsonResponse({ error: "Could not flag the payment for manual review." }, 500);
      }
    ...
    const fulfillmentUpdateError = await updatePaymentWithFallback(adminClient, paymentRecordId, [
      {
        fulfillment_error: null,
        fulfillment_status: "fulfilled",
        numbers_assigned_at: occurredAt,
        reservation_expires_at: null,
        status: "paid",
      },
      {
    ...
    if (fulfillmentUpdateError) {
      return jsonResponse({ error: "Could not finalize the payment fulfillment." }, 500);
    }
  }
