...
WITH latest_draw_prizes AS (
  SELECT DISTINCT ON (draws.promotion_id)
    draws.promotion_id,
    draws.prize_pool
  FROM public.draws AS draws
  WHERE draws.promotion_id IS NOT NULL
  ORDER BY draws.promotion_id, draws.draw_date DESC, draws.created_at DESC
)
UPDATE public.promotions AS promotions
SET prize_amount = COALESCE(promotions.prize_amount, latest_draw_prizes.prize_pool, 0)
FROM latest_draw_prizes
WHERE latest_draw_prizes.promotion_id = promotions.id
  AND promotions.prize_amount IS NULL;
...
ALTER TABLE public.promotions
  ALTER COLUMN prize_amount SET DEFAULT 0;

ALTER TABLE public.promotions
  ALTER COLUMN prize_amount SET NOT NULL;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'promotions_prize_amount_check'
      AND connamespace = 'public'::regnamespace
  ) THEN
    ALTER TABLE public.promotions
      ADD CONSTRAINT promotions_prize_amount_check
      CHECK (prize_amount >= 0);
  END IF;
END
$$;

DROP INDEX IF EXISTS public.promotion_numbers_promotion_ticket_idx;
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE public.promotion_numbers
      DROP CONSTRAINT promotion_numbers_ticket_range_check;
  END IF;
END
$$;
...
ALTER TABLE public.promotion_numbers
  ...
  CHECK (ticket_number BETWEEN 0 AND 999999);
...
BEGIN
  ...
  NEW.prize_amount := COALESCE(NEW.prize_amount, 0);
...
BEGIN
  ...
  remaining_numbers := 1000000 - assigned_numbers - active_pending_numbers;
